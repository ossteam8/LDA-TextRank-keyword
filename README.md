...
전처리 과정에서는 노동 집약적으로 만들어진 [불용어사전](stop.txt)와 [사용자사전](https://github.com/ossteam8/LDA-TextRank-keyword/blob/main/nnp.csv)을 이용하였습니다.
...
전처리를 위해 입력되는 문서(뉴스 기사들)는 ```["기사1","기사2","기사3",....,"기사N"]```의 형식이다.

명사 추출은 한글 형태소 분석기 ```Mecab```을 사용한다.

[사용자 단어 사전을 구축](#사용자-단어-사전-설치-방법)하여 형태소 분석이 잘 되지 않아 추출되지 않는 명사를 잘 인식할 수 있도록 돕는다.
